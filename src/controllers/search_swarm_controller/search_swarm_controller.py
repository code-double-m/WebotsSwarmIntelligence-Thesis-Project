...
class SwarmRobot():
    ...
    def render_map(self, ldr_data):
        ...
        grid_x = int((GOAL[0] + (GRID_SIZE_X * GRID_RESOLUTION)/2)/GRID_RESOLUTION)
        grid_y = int((GOAL[1] + (GRID_SIZE_Y * GRID_RESOLUTION)/2)/GRID_RESOLUTION)
        if 0 <= grid_x < GRID_SIZE_X and 0 <= grid_y < GRID_SIZE_Y:
            self.map[grid_x][grid_y] = 2
    ...
    def run(self):
        while self.robot.step(self.timestep) != -1:
            ...
            message = [self.ID, pos, rot, self.state, self.command, self.point_of_interest]
